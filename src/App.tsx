...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-[#fcfbfa] font-sans text-neutral-800 py-6 px-4 md:py-12 md:px-8 selection:bg-amber-200 selection:text-neutral-900">
      <div className="max-w-7xl mx-auto flex flex-col gap-6">
        
         {/* ================= HEADER SECTION ================= */}
        <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-neutral-200 pb-5 gap-3">
          <div>
            <div className="flex items-center gap-2">
              <div className="p-1.5 bg-amber-500 rounded-lg text-neutral-950 animate-pulse shadow-[0_0_12px_#f59e0b]">
            ...
            <p className="text-xs text-neutral-600 mt-1.5 font-bold">
              실시간 온라인 스트리밍 수신과 사운드 합성 믹서기능을 합친 프레스티지 비주얼 오디오 시스템
            </p>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-[10px] font-mono bg-white text-neutral-700 px-3 py-1.5 rounded-lg border border-neutral-200 shadow-sm font-extrabold">
        ...
        <main className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          ...
          <section className="lg:col-span-5 flex flex-col gap-5">
            ...
            <div className="bg-gradient-to-b from-[#f7f6f2] to-[#ebeae4] border-2 border-neutral-300 rounded-3xl p-6 shadow-[0_12px_40px_rgba(0,0,0,0.06)] relative overflow-hidden">
              {/* Premium timber top highlight line */}
              <div className="absolute top-0 inset-x-0 h-1 bg-amber-500" />
              
              <div className="flex items-center justify-between mb-5">
                <span className="text-[9px] font-mono text-neutral-600 font-extrabold uppercase tracking-widest leading-none">
                ...
                <button
                  ...
                  className={`relative px-4 py-2 rounded-xl flex items-center gap-2 text-xs font-mono font-bold cursor-pointer transition-all border ${
                    isPowerOn
                      ? 'bg-red-500 text-white border-red-600 shadow-[0_4px_16px_rgba(239,68,68,0.25)] font-extrabold'
                      : 'bg-white text-neutral-500 border-neutral-300 hover:text-neutral-800 hover:border-neutral-400 shadow-sm'
                  }`}
                  title={isPowerOn ? '라디오 시스템 전원 끄기' : '라디오 시스템 전원 켜기'}
                >
                  <Power className={`w-3.5 h-3.5 ${isPowerOn ? 'text-white animate-ping absolute' : ''}`} />
                  <Power className={`w-3.5 h-3.5 ${isPowerOn ? 'text-white z-10' : ''}`} />
                  <span>{isPowerOn ? 'POWER ON' : 'STANDBY'}</span>
                </button>
              </div>

              {/* VFD / CRT SCREEN CONSOLE HOUSING */}
              <div className="bg-neutral-950 border border-neutral-800 rounded-2xl p-4.5 shadow-inner mb-5 relative">
                ...
                <div className="flex justify-between items-start mb-4 gap-4">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[10px] font-mono text-neutral-400 tracking-wider font-semibold">주파수 대역</span>
                    <div className="flex items-baseline gap-1 mt-1 font-mono">
                      <span className={`text-4xl font-extrabold tracking-tighter transition-all ${
                        isPowerOn 
                      ...
                      <span className={`text-xs font-bold ${
                        isPowerOn ? 'text-amber-500/75' : 'text-neutral-800'
                      }`}>
                        MHz
                      </span>
                    </div>
                  ...
                  <div className="flex items-center gap-1.5 bg-neutral-900 border border-neutral-800 rounded-xl p-2.5">
                    <div className="flex flex-col text-right">
                      <span className="text-[9px] font-mono text-neutral-400 leading-none font-bold">SIGNAL BAND</span>
                      <span className={`text-[10px] font-mono font-bold mt-1.5 leading-none ${
                        isPowerOn 
                          ? (hasValidSignal ? 'text-emerald-400 font-extrabold' : 'text-red-500') 
                          : 'text-neutral-600'
                      }`}>
                    ...
                    <div className="relative">
                      <Disc 
                        className={`w-7 h-7 text-neutral-700 transition-colors ${
                ...
                <div className="border-t border-neutral-900 pt-3 flex flex-col gap-1.5 min-h-[55px]">
                  {isPowerOn ? (
                    currentStation ? (
                      <div>
                        ...
                        <div className="flex items-center justify-between gap-2.5">
                          <span className="text-xs font-bold text-amber-500/90 flex items-center gap-1 font-mono uppercase bg-amber-950/40 border border-amber-900/30 px-1.5 py-0.5 rounded">
                        ...
                        <h2 className="text-sm font-semibold text-neutral-100 mt-1 text-wrap break-all">
                          {currentStation.name}
                        </h2>
                      ...
                      <div className="text-neutral-400 flex flex-col gap-0.5 justify-center h-full py-1">
                    ...
                    <div className="text-neutral-600 text-xs py-2 font-mono flex items-center justify-center gap-1.5 text-center h-full">
                ...
                {isPowerOn && (
                  <div className="mt-2.5 flex items-center justify-between border-t border-neutral-900 pt-2 px-1">
                    <div className="flex items-center gap-3">
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-[8px] font-mono text-neutral-500 font-bold">PLAY</span>
                      </div>
                      <div className="flex items-center gap-1">
                        <span className={`w-1.5 h-1.5 rounded-full ${!isPlaying ? 'bg-amber-500 shadow-[0_0_4px_#f59e0b]' : 'bg-neutral-805'}`} />
                        <span className="text-[8px] font-mono text-neutral-500 font-bold">PAUSE</span>
                      </div>
                    </div>
                    {isLoadingStream && (
              ...
              {isPowerOn && (
                <div className="mt-4 flex items-center justify-between rounded-xl bg-white border border-neutral-300 p-2.5 px-3 shadow-sm">
                  <div className="flex items-center gap-1.5">
                    <Music className="w-3.5 h-3.5 text-neutral-600 font-bold" />
                    <span className="text-[10px] font-sans text-neutral-700 font-extrabold">재생 제어</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <button
                      id="radio-play-btn"
                      onClick={() => setIsPlaying(true)}
                      className={`px-3.5 py-1.5 text-[10px] font-extrabold font-mono rounded-lg cursor-pointer transition-all ${
                        isPlaying ? 'bg-amber-500 text-neutral-950 font-black shadow-md' : 'bg-neutral-100 text-neutral-500 hover:text-neutral-800 hover:bg-neutral-200'
                      }`}
                    >
                      PLAY
                    </button>
                    <button
                      ...
                      className={`px-3.5 py-1.5 text-[10px] font-extrabold font-mono rounded-lg cursor-pointer transition-all ${
                        !isPlaying ? 'bg-neutral-800 text-white font-black shadow-md' : 'bg-neutral-100 text-neutral-500 hover:text-neutral-800 hover:bg-neutral-200'
                      }`}
                    >
                      PAUSE
                    </button>
        ...
        {/* ================= FOOTER ADVISORY ================= */}
        <footer className="text-center py-6 border-t border-neutral-200 mt-6 select-none font-mono text-[10px] text-neutral-500 flex flex-col sm:flex-row items-center justify-between gap-3 font-semibold">
          <span>
